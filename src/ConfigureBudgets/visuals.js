...
function Visuals() {
  ...
  useEffect(() => {
    ...
    if (selectedMonth) {
      axios
        .get(`http://159.203.70.22:3002/get-budgets/${userId}?month=${selectedMonth}`, {
          headers: {
  ...
  useEffect(()=>{
    ...
    if (selectedMonth) {
      axios
        .get(`http://159.203.70.22:3002/get-expenses/${userId}?month=${selectedMonth}`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        })
    ...
    <main className="center" id="main" aria-label="main">
      ...
      <article className="chart">
       ...
       <article className="chart">
        ...
        <div>
          <p>Data is not present. Please enter the data from the</p>
        </div>
       )
